...
from .form import UserRegisterForm, UserUpdateForm, ProfileUpadateForm
...
def profile(request):
    if request.method == 'POST':
        ...
        if u_form.is_valid() and p_form.is_valid:
            ...
            messages.success(request, '{} votre compte a bien été modifié'.format(username))
            return redirect('user-profile')
    else:
        u_form = UserUpdateForm(instance=request.user)
        p_form = ProfileUpadateForm(request.FILES, instance=request.user.profile)
